<template>
  <n-tabs type="line" animated>
    <n-tab-pane name="form" tab="动态表单">
      <n-card title="分组表单" style="margin-top: 20px;">
        <DynamicForm :schema="groupSchema" v-model="groupFormData" />
      </n-card>

      <n-card title="多列布局" style="margin-top: 20px;">
        <DynamicForm :schema="layoutSchema" v-model="layoutFormData" />
      </n-card>

      <n-card title="不可收缩分组" style="margin-top: 20px;">
        <DynamicForm :schema="nonCollapsibleSchema" v-model="nonCollapsibleFormData" />
      </n-card>

      <n-card title="混合分组与非分组" style="margin-top: 20px;">
        <DynamicForm :schema="mixedSchema" v-model="mixedFormData" />
      </n-card>

      <n-card title="全面布局示例" style="margin-top: 20px;">
        <DynamicForm :schema="demoLayoutSchema" v-model="demoLayoutFormData" />
      </n-card>
    ...
    <n-tab-pane name="table" tab="动态表格">
      <n-card title="基础表格">
        <DynamicTable :columns="basicColumns" :data="basicData" width="600px" height="200px" />
      </n-card>

      <n-card title="分页表格" style="margin-top: 20px;">
        <DynamicTable :columns="pageColumns" :data="pageData" :pagination="{ pageSize: 5 }" width="80%"
          height="300px" />
      </n-card>

      <n-card title="自定义渲染" style="margin-top: 20px;">
        <DynamicTable :columns="customColumns" :data="customData" :width="700" :height="250" />
      </n-card>
    </n-tab-pane>
  </n-tabs>
</template>
...
import { NTabs, NTabPane, NCard, NSpace, NButton, useMessage } from 'naive-ui'
...
import { FormApi } from '@/components/DynamicComponent/DynamicForm/formApi'
